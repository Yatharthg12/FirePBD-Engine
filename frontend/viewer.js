...
class BlueprintViewer {
  ...
  render() {
    ...
    // Draw wall skeleton first so room geometry reads clearly.
    if (this.model.walls?.length) {
      ctx.save();
      ...
      for (const wall of this.model.walls) {
        const a = this.worldToCanvas(wall.x1, wall.y1);
        const b = this.worldToCanvas(wall.x2, wall.y2);
        ctx.beginPath();
        ctx.moveTo(a.x, a.y);
        ctx.lineTo(b.x, b.y);
        ctx.stroke();
