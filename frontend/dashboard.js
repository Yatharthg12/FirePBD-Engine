...
function _evacSummary(results) {
  const source = results?.evacuation ? results : (results?.summary ? results.summary : results || {});
  const evacuation = source.evacuation || source.evacuation_summary || {};
  const simulation = source.simulation || {};
  return {
    total_agents: evacuation.total_agents ?? simulation.total_agents ?? source.occupant_count ?? 0,
    evacuated: evacuation.evacuated ?? evacuation.survivors ?? 0,
    incapacitated: evacuation.incapacitated ?? 0,
    dead: evacuation.dead ?? 0,
    casualties: evacuation.casualties ?? ((evacuation.incapacitated ?? 0) + (evacuation.dead ?? 0)),
  };
}
...
function _updateScenarioCard(zoneId, routeZoneIds) {
  ...
  const marker = _selectedMarkerPoint
    ? `Ignition point: (${_selectedMarkerPoint.x.toFixed(1)}, ${_selectedMarkerPoint.y.toFixed(1)})<br/>`
    : '';
  card.innerHTML = `
    <div class="scenario-title">Scenario</div>
    <div class="scenario-body">
      ${marker}
      Ignition zone: <b>${zoneId}</b><br/>
...
function _applyRoutePreview(zoneId, worldPoint = null) {
  ...
  if (window.blueprintViewer) {
    window.blueprintViewer.setSelection(actualZoneId);
...
function switchTab(tab) {
  ...
  document.querySelectorAll('.tab-panel').forEach(panel => {
    const isActive = panel.id === _tabPanelId(tab);
    panel.classList.toggle('active', isActive);
    panel.hidden = !isActive;
    panel.style.display = isActive ? 'flex' : 'none';
  });
  if (tab === 'analysis' && _results) buildAnalysisCharts();
  if (tab === 'montecarlo' && _results) buildMonteCarloCharts();
  if (tab === 'optimization' && _results) buildOptimizationTab();
}
...
async function runSimulation() {
  ...
  const body = {
    ...
    ignition_point: _selectedMarkerPoint ? { x: _selectedMarkerPoint.x, y: _selectedMarkerPoint.y } : null,
    n_steps: parseInt(document.getElementById('simSteps').value) || 90,
    dt_s: 5.0,
    occupant_count: parseInt(document.getElementById('simOccupantCount').value) || 30,
  ...
  simLog(`\n[SIM] Starting simulation — ${body.n_steps} steps, ignition: ${body.ignition_zone}, occupants: ${body.occupant_count}`);
...
function renderStepSnapshot(msg, fromHistory = false) {
  ...
  const resolved_agents = (em.evacuated ?? 0) + (em.incapacitated ?? 0) + (em.dead ?? 0);
  const evac_pct = ((resolved_agents) / total_agents * 100).toFixed(1);
  ...
  const activeCount = em.active ?? ((em.total_agents ?? 0) - resolved_agents);
  const statActive = document.getElementById('statActive');
  if (statActive) statActive.textContent = `${Math.max(activeCount, 0)} active`;
...
function handleStepUpdate(msg) {
  ...
  renderStepSnapshot(msg);
}

async function _playReplayFrames(frames) {
  if (!frames.length) return;
  _stopPlaybackTimer();
  const delayMs = Math.max(25, Math.min(120, Math.round(700 / Math.max(frames.length / 12, 1))));
  for (const frame of frames) {
    renderStepSnapshot(frame, true);
...
async function handleSimulationComplete(msg) {
  ...
  _results = fetchedResults || msg.summary || _results;
  if (_results) updateResultsUI(_results);

  const replayFrames = _stepHistory.length ? [..._stepHistory] : [];
  if (replayFrames.length) {
    setLoadingProgress(97, 'Replaying simulation…');
    await _playReplayFrames(replayFrames);
  }
...
function updateResultsUI(results) {
  ...
  const ev = _evacSummary(results);

  const totalAgents = ev.total_agents ?? results.occupant_count ?? 0;
  const survivors = ev.evacuated ?? 0;
  const casualties = ev.casualties ?? ((ev.incapacitated ?? 0) + (ev.dead ?? 0));
  const unresolved = results.unresolved_agents ?? ev.unresolved ?? 0;
  const rset = rr.rset_s ?? summ.rset_s;
  const aset = rr.aset_s ?? summ.aset_s;
  const margin = rr.rset_aset_margin_s ?? summ.aset_rset_margin_s;
  const safety = rr.margin_assessment || (margin == null ? 'UNKNOWN' : margin >= 120 ? 'ADEQUATE' : margin >= 0 ? 'MARGINAL' : 'INADEQUATE');
  ...
  // RSET / ASET
  document.getElementById('rsetValue').textContent = rset == null ? 'Not reached' : `${Number(rset).toFixed(1)}s`;
  document.getElementById('asetValue').textContent = aset == null ? 'Not reached' : `${Number(aset).toFixed(1)}s`;
  document.getElementById('marginValue').textContent = margin !== null && margin !== undefined ? `${Number(margin).toFixed(1)}s` : '—';
  const mEl = document.getElementById('marginStatus');
  mEl.textContent = `${safety} (required ≥ 120s)`;
  mEl.style.color = safety === 'ADEQUATE' ? 'var(--safe)' : safety === 'MARGINAL' ? 'var(--warn)' : 'var(--danger)';

  // Outcome summary
  const outcomeAgents = document.getElementById('outcomeAgents');
  const outcomeSurvivors = document.getElementById('outcomeSurvivors');
  const outcomeCasualties = document.getElementById('outcomeCasualties');
  const outcomeSafety = document.getElementById('outcomeSafety');
  if (outcomeAgents) outcomeAgents.textContent = `${totalAgents}`;
  if (outcomeSurvivors) outcomeSurvivors.textContent = `${survivors}`;
  if (outcomeCasualties) outcomeCasualties.textContent = `${casualties}`;
  if (outcomeSafety) outcomeSafety.textContent = unresolved > 0 ? `INCOMPLETE (${unresolved} active)` : safety;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const smokeBtn = document.getElementById('toggleSmoke');
  if (smokeBtn) smokeBtn.classList.remove('active');
  if (window.simRenderer) window.simRenderer.layers.smoke = false;
  switchTab('dashboard');
