...
async def _run_simulation_task(sim_id: str, req: SimulationRequest) -> None:
    ...
    try:
        ...
        state.log_event(
            "Agent pipeline: blueprint → topology → fire → evacuation → risk → optimization → report"
        )

        # ── Build topology ────────────────────────────────────────────────────
        state.log_event("TopologyAgent: building navigation graph and simulation grid")
        topo = TopologyAgent(cell_size_m=req.cell_size_m)
        graph, grid = topo.build(model, auto_repair=True)

        # ── Fire setup ────────────────────────────────────────────────────────
        state.log_event("FireAgent: preparing ignition and spread model")
        ...
        point = None
        if req.ignition_point and "x" in req.ignition_point and "y" in req.ignition_point:
            point = (
                float(req.ignition_point["x"]) + grid.world_origin_offset[0],
                float(req.ignition_point["y"]) + grid.world_origin_offset[1],
            )
        ignited_count = fire_sim.ignite_zone(ignition_zone, seed_point=point)
        state.log_event(f"Fire ignited in zone '{ignition_zone}' ({ignited_count} cells)")

        # ── Evacuation setup ──────────────────────────────────────────────────
        state.log_event("EvacuationAgent: populating occupants and routing exits")
        evac_sim = EvacuationSimulator(graph, model, dt_s=req.dt_s)
        if req.occupant_count and req.occupant_count > 0:
            evac_sim.populate_randomly(req.occupant_count, seed=int(state.simulation_id[:8], 16))
        else:
        ...
        for step in range(req.n_steps):
            ...
            if rset is not None and state.rset_s is None and evac_metrics.get("all_resolved"):
                state.log_rset()
            ...
            if step % WS_STREAM_INTERVAL_STEPS == 0:
                ...
                for person in evac_sim.people:
                    ...
                    px = person.x if person.x is not None else (zone.centroid.x if zone else 0.0)
                    py = person.y if person.y is not None else (zone.centroid.y if zone else 0.0)
                    agents.append({
                        "id": person.id,
                        "x": round(px, 2),
                        "y": round(py, 2),
                ...
                snap = {
                    ...
                    "grid_origin": {
                        "x": grid.world_origin_offset[0],
                        "y": grid.world_origin_offset[1],
                    },
                    "zone_status": {
                        zid: {
                            "danger": s["danger"],
        ...
        # ── Risk analysis ─────────────────────────────────────────────────────
        state.log_event("RiskAgent: calculating RSET, ASET, and scenario risk")
        ...
        if req.run_monte_carlo:
            state.log_event("RiskAgent: running Monte Carlo uncertainty analysis")
        ...
        if req.run_optimization:
            state.log_event("OptimizationAgent: generating improvement recommendations")
        ...
        if req.generate_report:
            try:
                state.log_event("ReportAgent: generating PDF report")
                gen = ReportGenerator(
        ...
        unresolved_agents = evac_summary.get("unresolved", 0)
        outcome_status = "incomplete" if unresolved_agents else "complete"
        if unresolved_agents:
            state.log_event(f"Simulation ended with {unresolved_agents} unresolved agents")
        ...
        sim["results"] = {
            ...
            "agent_pipeline": [
                ...
                "RiskAgent",
                "OptimizationAgent",
                "ReportAgent" if req.generate_report else "ReportAgent (skipped)",
            ],
        }
