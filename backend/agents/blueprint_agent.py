...
class BlueprintAgent:
    ...
    def _post_process(self, model: BuildingModel) -> None:
        ...
        # Openings close to the perimeter often represent the actual exit door.
        for opening in model.openings.values():
            if not opening.midpoint or opening.is_exit_door:
                continue
            mx, my = opening.midpoint
            near_edge = (
                mx <= bbox[0] + margin * 1.5 or
                my <= bbox[1] + margin * 1.5 or
                mx >= bbox[2] - margin * 1.5 or
                my >= bbox[3] - margin * 1.5
            )
            if not near_edge:
                continue
            candidate_ids = [opening.zone_a, opening.zone_b]
            best_zone = None
            best_score = float("inf")
            for zid in candidate_ids:
                zone = model.zones.get(zid)
                if zone is None:
                    continue
                score = min(
                    abs(zone.centroid.x - bbox[0]),
                    abs(zone.centroid.x - bbox[2]),
                    abs(zone.centroid.y - bbox[1]),
                    abs(zone.centroid.y - bbox[3]),
                )
                if score < best_score:
                    best_score = score
                    best_zone = zone
            if best_zone is not None and not best_zone.is_exit:
                best_zone.is_exit = True
                logger.debug(f"Marked perimeter opening zone '{best_zone.id}' as exit")
...
class ImagePipeline:
    ...
    def process(self, img_path: str) -> BuildingModel:
        ...
        wall_lines = detect_wall_lines(wall_binary, min_line_length=24, max_line_gap=8)
        ...
        for idx, (x1, y1, x2, y2) in enumerate(wall_lines):
            model.add_wall(WallSegment(x1 * scale, y1 * scale, x2 * scale, y2 * scale))
        ...
        # Add wall segments from wall mask boundaries if Hough lines were sparse.
        if len(model.walls) < 20:
