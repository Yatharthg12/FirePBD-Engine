...
class FireSimulator:
    ...
    def ignite_zone(self, zone_id: str, seed_point: Optional[Tuple[float, float]] = None) -> int:
        """Ignite a small local cluster inside a zone. Returns count of cells ignited."""
        grid = self.grid
        ...
        if seed_point is not None:
            target_r, target_c = grid.world_to_cell(seed_point[0], seed_point[1])
            ordered = sorted(
                cells,
                key=lambda rc: abs(rc[0] - target_r) + abs(rc[1] - target_c),
            )
        else:
            zone_cells = cells
            target_r = sum(r for r, _ in zone_cells) / len(zone_cells)
            target_c = sum(c for _, c in zone_cells) / len(zone_cells)
            ordered = sorted(
                cells,
                key=lambda rc: abs(rc[0] - target_r) + abs(rc[1] - target_c),
            )

        seed_count = min(4, max(1, len(cells) // 80 + 1))
        for r, c in ordered[:seed_count]:
