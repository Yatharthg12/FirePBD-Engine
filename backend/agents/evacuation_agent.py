...
class EvacuationSimulator:
    ...
    def populate_from_model(self, seed: Optional[int] = None) -> int:
        ...
        for zone in occupied_zones:
            ...
            for _ in range(n):
                ...
                self._place_person_in_zone(p, zone)
                self.add_person(p)
                count += 1
    ...
    def populate_randomly(
        ...
        """
        Populate a specific number of occupants distributed across non-exit zones.
        Zones are selected by area so larger rooms receive more occupants.
        """
        if seed is not None:
            random.seed(seed)
        ...
        weights = [max(zone.area, 1.0) for zone in occupied_zones]
        count = 0
        for _ in range(max(0, agent_count)):
            zone = random.choices(occupied_zones, weights=weights, k=1)[0]
            p = Person(
                id=f"P{count:04d}",
                current_zone=zone.id,
            )
        ...
        logger.info(f"Populated {count} agents from random occupant scenario")
        return count

    def add_person(self, person: Person) -> None:
        self.people.append(person)

    def _place_person_in_zone(self, person: Person, zone) -> None:
        pt = self._random_point_in_zone(zone)
        person.x = pt[0]
        person.y = pt[1]

    def _random_point_in_zone(self, zone) -> tuple:
        min_x, min_y, max_x, max_y = zone.polygon.bounds
        for _ in range(24):
    ...
    def _move_person(self, person: Person) -> None:
        ...
        person.current_zone = next_zone_id
        zone_next = self.model.zones.get(next_zone_id)
        if zone_next is not None:
            person.x, person.y = self._random_point_in_zone(zone_next)
        person.path.pop(0)
    ...
    def _step_metrics(
        ...
        active = sum(1 for p in self.people if p.is_active)
        ...
        unresolved = total - evacuated - incapacitated - dead
    ...
    def summary(self) -> dict:
        ...
        active = [p for p in self.people if p.is_active]
        ...
        return {
            "total_agents": total,
            "survivors": len(evacuated),
            "casualties": len(incapacitated) + len(dead),
            "evacuated": len(evacuated),
            "active": len(active),
            "unresolved": max(total - len(evacuated) - len(incapacitated) - len(dead), 0),
            ...
            "resolved_pct": round(100 * (len(evacuated) + len(incapacitated) + len(dead)) / max(total, 1), 1),
